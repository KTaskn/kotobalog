<template>
  <b-card class="sentence-card" :title="card.sentence" :sub-title="card.creator">
    ...
    <a class="twitter-share-button" v-bind:href="card.tweet_text">Tweet</a>
  </b-card>
</template>
  ...
  data () {
    return {
      card: {
        ...
        tweet_text: encodeURI('https://twitter.com/intent/tweet?text=' + this.format_tweet_text(this.sentence.sentence, this.sentence.creator) + ' ＠コトバノートで記録しました。')
      },
      show_text: false
    }
  },
